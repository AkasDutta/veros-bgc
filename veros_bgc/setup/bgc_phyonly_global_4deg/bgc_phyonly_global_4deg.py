...
class GlobalFourDegreeBGC(VerosSetup):
    ...
    def set_initial_conditions(self, state):
        ...
        vs.bgc_tracers = update_multiply(
            vs.bgc_tracers, at[:, :, :, :, 0], vs.maskT[..., npx.newaxis]
        )
    ...
    def set_diagnostics(self, state):
        ...
        output_frequency = 10 * 86400.0

        state.diagnostics["snapshot"].output_frequency = output_frequency

        state.diagnostics["overturning"].output_frequency = output_frequency
        state.diagnostics["overturning"].sampling_frequency

        state.diagnostics["energy"].output_frequency = output_frequency
        state.diagnostics["energy"].sampling_frequency = 86400

        snapshot_vars = ["bgc_tracers"]
        state.diagnostics["snapshot"].output_variables = snapshot_vars
